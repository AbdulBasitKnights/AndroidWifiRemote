...
class BillingService(
    ...
    override fun checkProStatus(onResult: (Boolean) -> Unit) {
        if (::mBillingClient.isInitialized && mBillingClient.isReady) {
            CoroutineScope(Dispatchers.IO).launch {
                val isPro = queryProStatusInternal()
                findUiHandler().post { onResult(isPro) }
            }
            return
        }

        val connectionListener = object : BillingClientConnectionListener {
            override fun onConnected(status: Boolean, billingResponseCode: Int) {
                removeBillingClientConnectionListener(this)
                if (!status) {
                    onResult(false)
                    return
                }
        ...
        addBillingClientConnectionListener(connectionListener)
    }

    private suspend fun queryProStatusInternal(): Boolean {
        if (!::mBillingClient.isInitialized || !mBillingClient.isReady) return false

        val proSkus = (subscriptionSkuKeys + nonConsumableKeys).toSet()
        val inAppResult: PurchasesResult = mBillingClient.queryPurchasesAsync(
            QueryPurchasesParams.newBuilder()
                .setProductType(BillingClient.ProductType.INAPP)
                .build(),
        ...
        val purchases = inAppResult.purchasesList + subsResult.purchasesList
        return purchases.any { purchase -> isActiveProPurchase(purchase, proSkus) }
    }

    private fun isActiveProPurchase(purchase: Purchase, proSkus: Set<String>): Boolean {
        val stateOk =
            purchase.purchaseState == Purchase.PurchaseState.PURCHASED ||
                purchase.purchaseState == Purchase.PurchaseState.PENDING
        return stateOk && purchase.products.any { it in proSkus }
    }
    ...
    override fun onPurchasesUpdated(billingResult: BillingResult, purchases: List<Purchase>?) {
        ...
        IAPActivity.billingCode = billingResult.responseCode ?: -1
        IAPActivity.billingMessage = billingResult.debugMessage ?: ""
            ...
            when {
                billingResult.isOk() -> {
                    ...
                    val productDetails: ProductDetails? =
                        productDetailsList.productDetailsList.find { it.productId == this }
